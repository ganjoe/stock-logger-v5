...
class StatusCommand(ICommand):
    ...
    def execute(self, ctx: CLIContext, args: List[str]) -> CommandResponse:
        try:
            ...
            if args:
                ...
                try:
                    ...
                    if len(args) == 1 and not args[0].startswith("{"):
                        ...
                        return CommandResponse(success=False, message="Invalid JSON filter format.", error_code="JSON_ERROR")
            ...
            return CommandResponse(success=True, payload=data_dict, message=msg)
            
        except Exception as e:
            return CommandResponse(success=False, payload=None, message=f"Error: {e}", error_code="FETCH_ERROR")
...
class QuoteCommand(ICommand):
    ...
    def execute(self, ctx: CLIContext, args: List[str]) -> CommandResponse:
        if not args:
            return CommandResponse(success=False, message="Usage: quote SYMBOL", error_code="INVALID_ARGS")
        ...
        try:
            ...
            return CommandResponse(success=False, message=f"Quote Error: {str(e)}", error_code="QUOTE_ERROR")
...
class ChartCommand(ICommand):
    ...
    def execute(self, ctx: CLIContext, args: List[str]) -> CommandResponse:
        if not args:
            return CommandResponse(success=False, message="Usage: chart SYMBOL [JSON_PAYLOAD]", error_code="INVALID_ARGS")
        ...
        try:
            ...
            return CommandResponse(success=False, message=f"Chart Error: {str(e)}", error_code="CHART_ERROR")
